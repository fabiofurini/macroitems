...
\subsection{The open-pit instances}

On MineLib the same effect is much larger. Table~\ref{tab:minelib} gives the
whole comparison, at ten capacities and a budget of 150 seconds per method.

\begin{table}[htbp]
\centering
\caption{MineLib: total time for ten capacities. \emph{best solver} is the
fastest of the three dual simplex codes; \textsc{to} means that none of them
finished a single capacity within 150~s.}
\label{tab:minelib}
\begin{tabular}{lrrrrrr}
\toprule
Instance & $n$ & $m$ & path (s) & Newton (s) & best solver (s) & ratio\\
\midrule
newman1           & $1\,060$   & $3\,922$     & 0.04  & 0.15  & 0.1   & $1.4\times$\\
zuck\_small       & $9\,400$   & $145\,640$   & 0.45  & 2.17  & 19.6  & $43.8\times$\\
kd                & $14\,153$  & $219\,778$   & 1.16  & 2.19  & 42.0  & $36.1\times$\\
zuck\_medium      & $29\,277$  & $1\,271\,207$& 3.16  & 18.88 & 299.5 & $\mathbf{94.9\times}$\\
p4hd              & $40\,947$  & $738\,609$   & 11.36 & 7.01  & \textsc{to} & ---\\
marvin            & $53\,271$  & $650\,631$   & 3.34  & 5.24  & 25.7  & $7.7\times$\\
w23               & $74\,260$  & $764\,786$   & 12.41 & 14.88 & \textsc{to} & ---\\
zuck\_large       & $96\,821$  & $1\,053\,105$& 25.00 & 30.91 & \textsc{to} & ---\\
sm2               & $99\,014$  & $96\,642$    & 2.38  & 1.45  & 1.2   & $0.5\times$\\
mclaughlin\_limit & $112\,687$ & $3\,035\,483$& 46.50 & 39.91 & \textsc{to} & ---\\
\bottomrule
\end{tabular}
\end{table}
...
\emph{Where the solvers finish, the margin is an order of magnitude or more}:
a median of $21.9\times$ and a maximum of $94.9\times$, on \code{zuck\_medium},
where the path returns the whole value function in $3.2$ seconds against
$299$ seconds for a warm-started dual simplex.

\emph{On four of the ten instances no simplex code finished a single
capacity} within 150 seconds, while the path computed \emph{every} capacity
in 11 to 47 seconds. On \code{mclaughlin\_limit} --- $112\,687$ blocks and
$3\,035\,483$ precedences --- that is $46.5$ seconds for the entire value
function against no answer at all.

\emph{The one instance the solvers win is the sparsest}, and that is the
point. \code{sm2} has $99\,014$ items but only $96\,642$ arcs, a density
below 1, and there a dual simplex is twice as fast as the path. It is the
largest instance in the collection by item count and the only one on which
the combinatorial method loses --- which is the density thesis of
Section~\ref{sec:methods} stated by a single instance.
...
$w_i|\lambda_r-\lambda_h|$, and on real instances they are larger by a wide
margin: across the 23 benchmark instances the median item gains a factor of
$44.5$, and across the eight open-pit instances on which it is computable, a
factor of $130.9$ --- three times as much, the difference being the density of
the precedence graph, since the denser it is the more a forced item drags in
with it. Since a
reduced cost fixes a variable only when it exceeds the gap to an incumbent,
that is the difference between a bound that fixes nothing and one that fixes
much of the instance.

The two are not comparable in cost. The canonical values follow from the
canonical sequence for nothing; the face-wide ones need one minimum cut per
item, $0.8$ to $19$ seconds on the benchmark and up to $333$ on the largest
open-pit instances, against $1$ millisecond to $0.4$ seconds for the canonical
dual certificate itself. They are what one
computes at a node worth the effort, not at every node.
...
A third measurement is worth recording because it decides how much room the
face-wide values have: the primal optimum is unique on 28 of the 29 instances
on which the dimensions can be computed --- $k_0=1$ and
$\dim\mathcal X^\star=0$ on all 23 benchmark instances and on five of the six
open-pit ones. The exception is zuck\_large, whose split macroitem does admit
an intermediate optimal closure, giving $k_0=2$ and $\dim\mathcal X^\star=1$;
it is also the instance with the largest dual face,
$\dim\mathcal D^\star=1\,030\,216$, as the trade between the two dimensions
requires. Intermediate optimal closures are therefore possible but rare, and
where they are absent the dual face is large --- which is the room the
face-wide reduced costs exploit.
...
The tables below are generated from the raw CSV files in
\code{experiments/results/} by \code{experiments/make\_tables.py}; no number
is hand-typed. Table~\ref{tab:structure} is the structural characterisation of
every instance, Table~\ref{tab:methods} the complete method comparison, and
Table~\ref{tab:recommendation} the recommendation those numbers support.

In Table~\ref{tab:structure}, $k$ is the number of macroitems and $q$ the last
one with positive ratio; \emph{largest} is the size of the largest macroitem;
$|\mathcal H|$, \emph{persist.} and \emph{gap bound} are taken at the
instance's own capacity, where it has one.

In Table~\ref{tab:methods}, \emph{first} is the cost of answering one capacity
from nothing, \emph{per extra} the marginal cost of each further capacity, and
\emph{break-even} the number of capacities beyond which computing the whole
canonical path is the cheaper choice. A status of \code{timeout} means the
method did not finish one capacity within the budget, which on the largest
instances is itself the result.

{\footnotesize
\input{../experiments/results/tables/structure}
}

{\footnotesize
\input{../experiments/results/tables/methods}
}

\input{../experiments/results/tables/recommendation}
