...
$44.5$, with the per-instance median between $27.9$ and $160.6$. Since a
...
item, $0.8$ to $19$ seconds on those instances, against $1$ to $37$
milliseconds for the canonical dual certificate itself. They are what one
...
face-wide values have. On \emph{all} 23 benchmark instances the primal optimum
is unique --- $k_0=1$ and $\dim\mathcal X^\star=0$ --- so the intermediate
optimal closures that would make the primal face positive-dimensional do not
occur there at all, and the dual faces are correspondingly large, with
$\dim\mathcal D^\star$ from $1\,482$ to $75\,812$.
